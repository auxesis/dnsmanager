...
class DnsmanagerControllerTest < Test::Unit::TestCase
	...
	def test_select_a_domain
		with_domainfile('something.com' => {'master' => '127.0.0.1'}) do
			faux_dig { post :zone, {:domain => 'something.com', :commit => 'Change Domain'} }
		end
	...
	def test_show_domain
		with_domainfile('example.org' => {'master' => '127.0.0.1'}) do
			faux_dig { get :zone, {}, {:domain => 'example.org'} }
		end
	...
	def test_show_domain_using_a_key
		with_domainfile('example.org' => {'master' => '127.0.0.1', 'key' => 'Kexample.org.+157.+random'}) do
			faux_dig(:wants_key => 'Kexample.org.+157.+random.private') do
	...
	def test_delete_a_record_with_key
		...
		with_domainfile('example.org' => {'master' => '127.0.0.1', 'key' => 'Kexample.org.+157+00000'}) do
			faux_dig(:wants_key => 'Kexample.org.+157+00000.private') do
				out = faux_nsupdate(:wants_key => 'Kexample.org.+157+00000.private') {
					get :delete,
				       { :id => "larry__A__192.168.1.1" },
				       { :domain => 'example.org' }
				}
	...
	def test_add_display
		with_domainfile('example.org' => {'master' => '127.0.0.1'}) do
			faux_dig { get :add, {}, {:domain => 'example.org'} }
		end
	...
	def test_add_post_with_key
		out = nil
		with_domainfile('example.org' => {'master' => '127.0.0.1', 'key' => 'Kexample.org.+157+00000'}) do
			out = faux_nsupdate(:wants_key => 'Kexample.org.+157+00000.private') { post :add,
			                      { :hostname => "hissy", :ttl => 300, :rrtype => "TXT", :rrdata => "he's an adder!", :commit => 'Add Record' },
			                      { :domain => 'example.org' }
			                    }
		end
	...
	def test_edit_unknown_record
		with_domainfile('example.org' => {'master' => '127.0.0.1'}) do
			faux_dig { get :edit, { :id => 'flahflah__A__127.0.0.1' }, { :domain => 'example.org'} }
		end
	...
	def test_edit_begin
		with_domainfile('example.org' => {'master' => '127.0.0.1'}) do
			faux_dig { get :edit, { :id => 'moe__A__192.168.1.3' }, { :domain => 'example.org' } }
		end
	...
	def test_mx_edit_begin
		with_domainfile('example.org' => {'master' => '127.0.0.1'}) do
			faux_dig { get :edit, { :id => '__MX__10+moe.example.org.' }, { :domain => 'example.org' } }
		end
